<?php
/*
Template Name: Custom Property Listing
*/
get_header();
			...
			<div class='row'>
				...
				<div class='property-list'>
					...
					if( have_posts() ) {
						...
						$orderby_parts = explode( ' ', $wp_query->query['orderby'] );
						$show_section_meta_value = ( count( $orderby_parts ) > 1 );
						$old_section_header = NULL;
						//END DAN CHANGES
						while( have_posts() ) {
							the_post();
							if( $property_layout == 'list' ) {
								//START DAN CHANGES
                                                                $property_city = get_post_meta( $post->ID, '_est_meta_city', true );
                                                                if ( $old_section_header != $property_city) {
                                                                    echo "<h1 class='section-header'>$property_city</h1>";
                                                                    $old_section_header = $property_city;
                                                                }
								//END DAN CHANGES
								get_template_part( 'layout-property-list-custom' );
							}
							else {
								if( $i % $card_columns == 0 ) {
									echo '<div class="row">';
