...
class bshSubmitPropertyPageOptions extends bshOptions {

	// 1.1 Constructor
	function __construct() {
		$args = array(
			'title'     => __( 'Submit Property Options', THEMENAME ),
			'post_type' => 'page',
			'template'  => 'template-bshSubmitPropertyPage.php',
			'context'   => 'normal',
			'priority'  => 'high'
		);
...
if ( is_admin() ) {
    add_action( 'load-post.php', 'call_bshSubmitPropertyPageOptions' );
    add_action( 'load-post-new.php', 'call_bshSubmitPropertyPageOptions' );
}
function call_bshSubmitPropertyPageOptions() {
	global $bshPostOptions;
	if( $bshPostOptions->get_page_template() == 'template-bshSubmitPropertyPage.php' ) {
    	return new bshSubmitPropertyPageOptions();
    }
